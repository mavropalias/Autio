(function () {
    ...
    var bookGroups = [
        { key: "group1", title: "All books", backgroundImage: darkGray },
        { key: "group2", title: "Unread books", backgroundImage: lightGray },
        { key: "group3", title: "Currently reading", backgroundImage: mediumGray }
    ];

    function addBook(book, cb) {
        /*list.push({
            group: { key: "group1", title: "All books", subtitle: "", backgroundImage: darkGray, description: "These are all your audiobooks" },
            title: book.title + " by " + book.author, subtitle: "Duration: " + book.durationStr, description: '', content: '', backgroundImage: book.cover
        });*/

        // check if book already exists and handle it
        var bookExists = false;

        list.forEach(function (existingBook) {
            if (existingBook.bookFolder === book.bookFolder) {
                // we already have this book folder,
                // we'll only update the files and cover image
                /*existingBook.author = book.author;
                if (book.coverUrl !== "/images/defaultCover.png") {
                    existingBook.cover = book.cover;
                    existingBook.coverUrl = book.coverUrl;
                }*/
                existingBook.cover = book.cover;
                existingBook.duration = book.duration;
                existingBook.durationStr = book.durationStr;
                existingBook.files = book.files;
                existingBook.title = book.title;
                existingBook.year = book.year;
                existingBook.timestamp = AB.timestamp;
        ...
        if (!bookExists) {
            book.group = bookGroups[0];
            book.progress = 0;
            book.progressStr = "00:00:00";
            book.currentPlayingFileArrayIndex = 0;
            book.timestamp = AB.timestamp;

            list.push(book);
            AB.addBookToLibrary(book);
            cb();
        } else {
            cb();
        }
    ...
    // Removes books that were not found on the last scan
    function cleanLibrary(item) {
        var booksToRemove = [];

        list.forEach(function (book, currentIndex) {
            if (book.timestamp !== AB.timestamp)
            {
                booksToRemove.push(currentIndex);
            }
        });

        AB.deleteSelectedItemsFromList(booksToRemove, list);
    }
